...
**`get_trip` takes a TEXT id and is total over it.** `p_trip_id` is `text`, not `uuid`: a trip id reaches the backend from a URL segment, so it is user input, and a malformed one must be answerable rather than fatal. A value that is not a well-formed uuid returns the ordinary `NOT_FOUND` envelope — the same answer as a well-formed id matching no trip, because to the caller they are the same fact: there is no such trip. Callers therefore do **not** need to pre-validate the id. (With `uuid`, Postgres rejected the argument at the type boundary with SQLSTATE 22P02 before the body ran, which reached the frontend as a transport failure and rendered the generic error card instead of the not-found card.) `get_seat_map` and `search_trips` still take `uuid` / `date` — they are only ever called with values the app already holds, never with raw URL input.
